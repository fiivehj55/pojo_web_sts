(function() {
	...
	Trex.Class.overwrite(Trex.Tool.SpecialChar, {
		oninitialized : function() {
			...
			this.resetWeave();
			this.weave.bind(this)(new Trex.Button(this.buttonCfg),
					new Trex.Menu.SpecialChar(this.menuCfg), i)
		}
	});
	Trex.MarkupTemplate
			.add(
	...
	Trex.Menu.SpecialChar = Trex.Class.create({
		...
		ongenerated : function() {
			var k = this.elMenu;
			var i = Trex.MarkupTemplate.get("menu.specialchar.input")
					.evaluateAsDom({});
		...
		onSelect : function(j) {
			...
			l.value += (!i.innerText || i.innerText == "&nbsp;" || i.innerText
					.trim() == "") ? "" : i.innerText;
			$tx.stop(j);
			l.focus();
			if ($tx.msie) {
